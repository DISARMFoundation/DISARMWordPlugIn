...
'Early binding. Project requires a reference to the Excel Object Library _
                see:  http://www.word.mvps.org/FAQs/InterDev/EarlyvsLateBinding.htm
'
' This procedure populates the ListBox oListPassed with all the techniques that apply to tactic strTacticName
'
...
With oApp
    ...
    For i = 1 To oWS_techniques.UsedRange.Cells(2, 4).End(xlDown).Row
        If oWS_techniques.Cells(i + 1, 4).Value = strTacticID Then
            arrTechniques(counter) = oWS_techniques.Cells(i + 1, 2).Value
            counter = counter + 1
        End If
    Next i
...
'
' Populate the list box oListPassed with the array of techniques
'
    ...
    With rngSearch
        ...
        If Not c Is Nothing Then
            ...
            If j > 0 Then
                ...
                For k = 1 To j
                    arrTableData(k, 3) = arrTechniques(2, k)
                    arrTableData(k, 2) = ReturnTacticName(arrTechniques(3, k))
                    strPhaseID = ReturnPhaseID(arrTechniques(3, k))
                    arrTableData(k, 1) = ReturnPhaseName(strPhaseID)
